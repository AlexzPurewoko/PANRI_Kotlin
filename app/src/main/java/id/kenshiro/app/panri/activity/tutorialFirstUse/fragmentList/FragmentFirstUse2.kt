...
class FragmentFirstUse2 : Fragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val layout = inflater.inflate(R.layout.acttutor_main_fragment_firstuse_2, container, false)
        val textView = layout.findViewById<TextView>(R.id.frag2_id_txt)
        textView.typeface = Typeface.createFromAsset(context?.assets, TutorialFirstUseActivityParams.TEXT_DEFAULT_FONT)
        textView.textColor = Color.WHITE
        textView.gravity = Gravity.CENTER
        textView.textSize = TutorialFirstUseActivityParams.TEXT_FRAGMENT2_CONTENT_SIZE
        return layout
    }

    companion object {
        fun newInstance(): Fragment {
            return FragmentFirstUse2()
        }
    }
}